...
async function createGoalParts(groupMesh, compoundShape) {
	...
	bottomWallMesh.position.set(
		0,
		-rightWallMesh.geometry.parameters.height,
		rightWallMesh.geometry.parameters.width / 2
	);
...
function createBottomGoal(position = { x: 10, y: -5, z: 13 }) {
	...
	mesh.collisionResponse = (mesh1) => {
		if (!isCollided) {
			...
			create3dText();
			for (let i = 0; i < 25; i++) {
				createCustomThing(Math.random(), {
					x: Math.random() * (19 - 1) + 1,
					y: -6,
					z: Math.random() * (22 - 4) + 4,
				});
			}
			initializeBoing();
