/*
Denne koden er basert på kodeeksempel ammoTerrain1/terrain_osm.js
https://source.coderefinery.org/3d/threejs_std/-/blob/main/src/del4/ammoTerrain1/terrain_osm.js
*/
...
export async function createGolfPlane() {
	...
	imageLoader.load("assets/textures/grass/grass_height.png", async (image) => {
		...
		let heightFieldData = createHeightFieldShape(
			heightData,
			terrainWidth,
			terrainHeight,
			terrainWidthExtents,
		...
		let delta =
			heightFieldData.terrainMinHeight +
			(heightFieldData.terrainMaxHeight - heightFieldData.terrainMinHeight) / 2;
...
function createHeightFieldShape(heightData, terrainWidth, terrainHeight, terrainWidthExtents, terrainDepthExtents) {
	...
	for (let j = 0; j < terrainHeight; j++) {
		for (let i = 0; i < terrainWidth; i++) {
			if (heightData[p] < terrainMinHeight) terrainMinHeight = heightData[p];
			if (heightData[p] >= terrainMaxHeight) terrainMaxHeight = heightData[p];
