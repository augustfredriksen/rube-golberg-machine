...
export function createButtonWithBelonings() {
	let button = createAmmoButton({ x: 0, y: 0, z: 0 }, { x: 21, y: 1.3, z: -23.5 }, 0.5, 0.5, 0.5, colorScheme.blue);
}
...
export function createAmmoGlass(rotation = { x: 0, y: 0, z: 0 }, position = { x: 10, y: 4, z: -23.5 }) {
	...
	let material = new THREE.MeshStandardMaterial({ color: colorScheme.blue, transparent: true, opacity: 0.2 });
	let mesh = new THREE.Mesh(geometry, material);
	mesh.position.set(position.x, position.y, position.z);
