...
async function createTrapDoorParts(groupMesh, compoundShape) {
	...
	let pillarGeometry = new THREE.BoxGeometry(0.4, 2.2, 0.4);
	...
	pillarMesh5.scale.set(1, 0.5, 1);
	groupMesh.add(pillarMesh5);

	let topGeometry = new THREE.BoxGeometry(3, 0.4, 0.4);
	let topMesh = new THREE.Mesh(topGeometry, material);
	topMesh.position.set(
		topMesh.position.x + 1.5,
		pillarMesh.geometry.parameters.height / 2 - topMesh.geometry.parameters.height / 2,
		topMesh.position.z
	);
